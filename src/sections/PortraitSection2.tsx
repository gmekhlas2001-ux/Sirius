...
export default function PortraitSection2() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Portrait - scale and slide
      scrollTl
        .fromTo(portrait,
          { scale: 1.10, x: '6vw', opacity: 0.7 },
          { scale: 1, x: 0, opacity: 1, ease: 'none' },
          0
        )
        .fromTo(portrait,
          { scale: 1, x: 0, opacity: 1 },
          { scale: 1.05, x: '-4vw', opacity: 0.4, ease: 'power2.in' },
          0.7
        );
  ...
  const headlineText = 'QUIET CONFIDENCE';
  ...
  return (
    ...
    >
      ...
      >
        <img
          src="/portrait_confidence_full.jpg"
          alt="Portrait"
      ...
      <div
        ref={overlayRef}
        className="absolute left-0 top-0 w-full md:w-[42vw] h-full"
        style={{
          background: 'linear-gradient(90deg, rgba(11,13,16,0.78), rgba(11,13,16,0))'
        }}
      />
      ...
      <div
        ref={textRef}
        className="absolute left-1/2 md:left-[9vw] bottom-[8%] md:top-1/2 -translate-x-1/2 md:-translate-x-0 md:-translate-y-1/2 w-[85vw] md:w-[30vw] px-6 md:px-0"
      >
        {/* Star Mark */}
